...
library(readxl)
MCKP_Data <- read_excel("OPER 623 - Heuristics/MCKP Lab Data.xlsx", col_names = FALSE, skip = 1)
...
popsize <- 20 #user tunable parameter for population size
...
for (i in 1:popsize) {
  SampleVector1<-order(MCKP_Data$X__1/MCKP_Data$X__2, decreasing = FALSE)
  SampleVector2<-order(MCKP_Data$X__1/MCKP_Data$X__3, decreasing = FALSE)
}
...
while(stop == 0) {
  ...
  if (dad.sector <= 30) {dad = sample(1:group1,1)}; #30% chance we grab from group 1
  if (dad.sector > 30 & dad.sector <=80) {dad = sample((group1+1):group2,1)}; #50% chance we grab from group 2
  if (dad.sector > 80 & dad.sector <=95) {dad = sample((group2+1):group3,1)}; #15% chance we grab from group 3
  ...
  lower <- round(datasize*.33);
  upper <- round(datasize*.66);
  crossover <- round(runif(1, min=lower, max=upper),0); #choose a random crossover point between 1/3 datasize and 2/3 datasize (i.e in middle of chromosomes)
  dads.ndx <- ndx[dad];
  moms.ndx <- ndx[mom];
  child <- c(population[dads.ndx,1:crossover],population[moms.ndx,(crossover+1):datasize]);
  ...
  #5) If Child is better then worst member of population replace worst member of population with child
